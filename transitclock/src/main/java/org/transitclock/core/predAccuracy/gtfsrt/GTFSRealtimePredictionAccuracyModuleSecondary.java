 ...
 *
 */
public class GTFSRealtimePredictionAccuracyModuleSecondary extends PredictionAccuracyModule {
	...
	private static final StringConfigValue gtfsTripUpdateUrl = new StringConfigValue(
			"transitclock.predAccuracy.gtfsTripUpdateUrlSecondary", "http://127.0.0.1:8091/trip-updates",
			"URL to access gtfs-rt trip updates.");

	private static StringConfigValue gtfsRealtimeHeaderKey =
			new StringConfigValue("transitclock.predictionAccuracy.apiKeyHeaderSecondary",
					null,
					"api key header value if necessary, null if not needed");

	private static StringConfigValue gtfsRealtimeHeaderValue =
			new StringConfigValue("transitclock.predictionAccuracy.apiKeyValueSecondary",
					null,
					"api key value if necessary, null if not needed");


  	private static ClassConfigValue translatorConfig =
		  new ClassConfigValue("transitclock.predAccuracy.RtTranslatorSecondary", null,
  ...
  private GTFSRealtimeTranslator translator = null;
	...
	class PredictionReadTimeKey {
		...
		private GTFSRealtimePredictionAccuracyModuleSecondary getOuterType() {
			return GTFSRealtimePredictionAccuracyModuleSecondary.this;
		}
	}
